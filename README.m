...
%
% This repository includes the code for the |barareaneg.m| Matlab function,
...
%
% This code can be downloaded from <https://github.com/kakearney/barareaneg-pkg/ Github>
% or the
...
% h = barareaneg(x, y, type, orient)
%
%% Description
%
% |h = barareaneg(x, y, type)| creates either a stacked bar (|type| =
% 'bar') or stacked area (|type| = 'area') plot, where the vector |x|
% indicates the coordinates of the independent variable and the matrix |y|
% indicates the values of to be stacked; rows of |y| correspond to |x|, and
% columns indicate values to be stacked.  The output |h| is a structure
% with handles to the plotted objects.
%
% |h = barareaneg(x, y, type, orient)| indicates whether bars/areas should
% stack vertically (|orient| = |'v'|) or horizontally (|orient| = |'h'|).

%% Examples
%
% We start with stacked data that includes 3 all-positive series and 2
% all-negative ones.
...
%% 
% Matlab's native bar plots display stacked negative data in a cumulative
% way, such that the blocks that change sign from the previous one overlay
% each other.  The |barareaneg| function instead splits the postivie and
% negative, which may be a more desirable way to display data, depending on
% the application.
...
% <https://github.com/kakearney/barareaneg-pkg/issues an issue> on GitHub and
% include:  
% 
